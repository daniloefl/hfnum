  ...
  for (int k = 0; k < _o.size(); ++k) {
    ...
    if (iterE >= 2) { // if there are at least 3 points, fit a parabola
      ...
      if ( ( (hF(0) > 0 && hF(1) > 0 && hF(2) > 0) ||
           ...
           (hF(0) - hF(1))/(hE(0) - hE(1)) >= 0.9*(hF(1) - hF(2))/(hE(1) - hE(2)) ) { // check if they are close to each other
        MatrixXld A;
        VectorXld F;
        A.resize(3, 3);
        F.resize(3);
        ...
        if (foundSolution) {
          std::cout << "Orbital " << k << ", interpolated parabola using (E, F) = "
    ...
    if (!foundSolution) {
      ...
      VectorXld Fd; // recalculate the function being minimized, because we are changing energy for orbital k independently from the others
      if (_isSpinDependent) {
      ...
      _dE[k] = 0;
      if (std::fabs(Fn(idx) - Fd(idx)) > 1e-16) {
